...
def refill_var(pool, batches, fx, fc, fp, fr, batch_size, sort_and_shuffle=True):
    line_pairs = []
    lines = []
    linex, linec, linep, liner = fx.readline(), fc.readline(), fp.readline(), fr.readline()
    ...
    while linex and linep and linec and liner:
        ...
        if len(lines) == 1000:
            ...
            line_pairs += res
            lines = []
        if len(line_pairs) + len(lines) == 16 * 1280:
            break
        linex, linec, linep, liner = fx.readline(), fc.readline(), fp.readline(), fr.readline()
...
def pair_iter(file_data, dev, num_wit, dict_bin, batch_size=128,
              max_seq_len=300,
    ...
    while True:
        if len(batches) == 0:
            if flag_generate:
                refill(batches, fx, batch_size,
                       max_seq_len=300, sort_and_shuffle=sort_and_shuffle)
            else:
                refill_var(pool, batches, fx, fc, fp, fr, batch_size,
