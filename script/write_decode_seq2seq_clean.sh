...
do
    ...
    cur_start=$(($j*$size))
    cur_end=$(($i * $size))
    cur_cmd='python decode_seq2seq_generate.py --random="'$random'" --model="seq2seq" --flag_bidirect=True --data_dir="'$folder_data'" --out_dir="'$folder_out'" --train_dir="'$folder_train'" --dev='$dev' --start='$cur_start' --end='$cur_end' --num_layers=3 --size=400 --num_cand=10 --num_pred=10 --beam_size=128 --batch_size=128'
    $(rm_file $cur_file)
