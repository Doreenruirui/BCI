...
do  
    ...
    do 
        ...
        vsj=$(($t * $size))
        vej=$(($j * $size))
        cur_cmd='python decode_seq2seq.py --random="'$random'" --model="lstm" --flag_bidirect=False --data_dir="'$folder_data'" --out_dir="'$folder_out'" --train_dir="'$folder_train'" --dev="'$dev'" --start='$vsj' --end='$vej' --max_seq_len=300 --num_layers=3 --size=512 --num_cand=10 --num_pred=10 --beam_size=128 --batch_size=64 --num_wit='$num_wit' --num_top=5 --prior=3 --prob_high=0.8 --prob_in=1.0 --flag_sum=0'
