...
class Model(object):
    ...
    def beam_step_lstm(self, time_step, state_inputs):
        inp = tf.reshape(tf.slice(self.encoder_inputs, [time_step, 0, 0], [1, -1, -1]),
                         [self.batch_size, -1])
    ...
    def beam_step_seq2seq(self, time_step, beam_seqs, state_inputs):
        ...
        with vs.variable_scope("Decoder", reuse=True):
            ...
            with vs.variable_scope("Attn", reuse=True):
                with vs.variable_scope("RNN", reuse=True):
                    out, attn_outputs = self.attn_cell.beam(rnn_out, state_inputs[-1], self.beam_size, self.batch_size)
        state_outputs = rnn_outputs + (attn_outputs, )
        return out, state_outputs

    def beam_loss(self, scope, output, vocab_size, model="seq2seq"):
        with vs.variable_scope(scope, reuse=True):
            ...
            if model == "lstm":
                logprob2d = tf.nn.log_softmax(logits2d)
            else:
                logprobs3d = tf.reshape(tf.nn.log_softmax(logits2d), [self.batch_size, self.beam_size, vocab_size])
        return logprobs3d

    def setup_beam(self, loss_scope, vocab_size, model='lstm'):
        time_0 = tf.constant(0)
        beam_seqs_0 = tf.ones([self.batch_size, self.beam_size, 1], dtype=tf.int32) * char2id['<sos>']
        beam_probs_0 = tf.zeros(shape=[self.batch_size, self.beam_size], dtype=tf.float32)
        top_seqs_0 = tf.zeros([self.batch_size, self.num_pred, 1], dtype=tf.int32)
        # top_probs_0 = tf.zeros([self.batch_size, self.num_pred, self.len_inp], dtype=tf.float32)
        ...
        def decode_cond(time, beam_seqs, beam_probs, top_seqs, perplex, *states):
            return time < self.len_inp

        def decode_step(time, beam_seqs, beam_probs, top_seqs, perplex, *states):
            if model == 'lstm':
            ...
            logprobs3d = self.beam_loss(loss_scope, decoder_outputs, vocab_size, model)

            total_probs = logprobs3d + tf.reshape(beam_probs, [self.batch_size, self.beam_size, 1])
            flat_total_probs = tf.reshape(total_probs, [self.batch_size, -1])

            next_beam_probs, top_indices = tf.nn.top_k(flat_total_probs, k=self.beam_size)

            next_bases = tf.floordiv(top_indices, vocab_size)
            next_mods = tf.mod(top_indices, vocab_size)

            flat_bases = tf.reshape(next_bases + tf.reshape(tf.range(self.batch_size),
                                                            [-1, 1])* self.beam_size,
                                    [-1])

            # cur_probs = tf.expand_dims(tf.slice(next_beam_probs, [0, 0], [-1, self.num_pred]), -1)
            cur_seqs = tf.expand_dims(tf.slice(next_mods, [0, 0], [-1, self.num_pred]), -1)
            next_top_seqs = tf.concat(2, [top_seqs, cur_seqs])
            # next_top_probs = tf.concat(1, [top_probs, cur_probs])

            gather_seqs = tf.reshape(tf.gather(tf.reshape(beam_seqs,
                                               [self.batch_size * self.beam_size, -1]),
                                    flat_bases),
                                     [self.batch_size, self.beam_size, -1])
            next_states = [tf.gather(state, flat_bases) for state in state_output]
            next_beam_seqs = tf.concat(2, [gather_seqs,
                                           tf.expand_dims(next_mods, -1)])

            cur_tok = tf.reshape(tf.slice(self.tgt_toks, [time + 1, 0], [1, -1]), [-1, 1, 1])
            batch_index = tf.tile(tf.reshape(tf.range(self.batch_size),
                                             [self.batch_size, 1, 1]),
                                  [1, self.beam_size, 1])
            beam_index = tf.tile(tf.reshape(tf.range(self.beam_size),
                                            [1, -1, 1]),
                                 [self.batch_size, 1, 1])
            toks = tf.tile(cur_tok, [1, self.beam_size, 1])
            perplex_prob = tf.reduce_max(tf.gather_nd(tf.exp(logprobs3d), tf.concat(2, [batch_index, beam_index, toks])), reduction_indices=-1)
            ...
            return [time + 1, next_beam_seqs, next_beam_probs, next_top_seqs, next_perplex] + next_states
    ...
    def decode_beam(self, session, src_toks, tgt_toks, src_mask, beam_size):
        ...
        output_feed = [self.beam_output, self.beam_scores, self.top_outputs, self.perplex]
