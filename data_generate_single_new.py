...
def load_data(batches, file_data, dev, batch_size=128, max_seq_len=300, flag_generate=False,
                    prob_high=1.0, start=0, end=-1, sort_and_shuffle=False):
    if prob_high == 1.0 or flag_generate:
...
def generate_sentence(sentence, prob_high):
    sentence = sentence[1:]
    nchar = len(sentence)
    nnoisy = np.int(np.floor(nchar * prob_high))
    index = np.arange(nchar)
    np.random.shuffle(index)
    voc_size = 27
    for i in range(nnoisy):
        cur_ind = index[i]
        cur_char = sentence[cur_ind]
        tmp_char = cur_char
        while tmp_char == cur_char:
            tmp_char = np.random.choice(voc_size, 1)[0] + 3
        sentence[cur_ind] = tmp_char
    sentence = [char2id['<sos>']] + sentence
    return sentence

def iter_data(batch, prob_high=1.0, flag_generate=False):
    x_tokens, y_tokens = batch
    if flag_generate:
        x_tokens = list(map(lambda sent: generate_sentence(sent, prob_high), x_tokens))
    x_pad = padded(x_tokens, 0)
    y_pad = padded(y_tokens, 0)
    source_tokens = np.transpose(np.array(x_pad), (1, 0))
    source_mask = (x_pad > 0).astype(np.int32)
