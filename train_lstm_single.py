...
from data_generate_single import id2char, load_data, iter_data
...
def create_model(session, vocab_size_char):
    model = model_lstm.Model(FLAGS.size, FLAGS.num_layers,
...
def validate(model, sess, batches):
    ...
    for batch in batches:
        source_tokens, source_mask, target_tokens = iter_data(batch)
...
def train():
    ...
    with tf.Session() as sess:
        ...
        test_batches = []
        load_data(test_batches, FLAGS.data_dir, FLAGS.dev, prob_high=FLAGS.prob_high,
              batch_size=FLAGS.batch_size, max_seq_len=FLAGS.max_seq_len)
        batches = []
        load_data(batches, FLAGS.data_dir, 'train', prob_high=FLAGS.prob_high,
                  max_seq_len=FLAGS.max_seq_len, batch_size=FLAGS.batch_size,
        ...
        while (FLAGS.epochs == 0 or epoch < FLAGS.epochs):
            ...
            for batch in batches:
                ...
                grad_norm, cost, param_norm = model.train(sess, source_tokens, source_mask,
