...
class Model(object):
            ...
            if model == 'seq2seq':
                ...
                self.decoder_inputs = embedding_ops.embedding_lookup(self.L_dec,
                                                                     tf.slice(self.tgt_toks,
                                                                              [0, 0],
                                                                              [self.len_inp - 1, -1]))
                self.decoder_inputs = tf.concat([embed_sos, self.decoder_inputs], axis=0)
    ...
    def setup_loss(self, output, vocab_size, size):
        with vs.variable_scope("Logistic"):
            logits2d = _linear(tf.reshape(output,
                                                   [-1, size]),
                                        vocab_size, True, 1.0)
            ...
            losses1d = tf.nn.softmax_cross_entropy_with_logits(logits=logits2d,
                                                               labels=labels) \
                       * tf.to_float(mask1d)
        ...
        with tf.variable_scope("Model", initializer=tf.uniform_unit_scaling_initializer(1.0)):
            ...
            if flag_bidirect:
                if not self.foward_only:
                    ...
                    self.encoder_output = tf.concat([self.enc_fw_out, self.enc_bw_out], axis=2)
            else:
    ...
    def beam_loss(self, scope, output, vocab_size):
        with vs.variable_scope(scope, reuse=True):
            ...
            logits2d = _linear(do2d, vocab_size, True, 1.0)
    ...
    def setup_beam(self, loss_scope, vocab_size):
        ...
        def decode_step(time, beam_seqs, beam_probs, perplex, top_seqs1, top_seqs2, top_seqs3, top_seqs4, *states):
            ...
            next_top_seqs3= tf.concat([top_seqs3, cur_seqs3], axis=2)
            ...
            next_top_seqs4 = tf.concat([top_seqs4, cur_seqs4], axis=2)
            ...
            toks_index = tf.concat([batch_index, beam_index, cur_tok], axis=2)
            perplex_prob = tf.reduce_max(tf.gather_nd(logprobs3d, toks_index),
                                         reduction_indices=-1)
            next_perplex = tf.concat([perplex, tf.reshape(perplex_prob, [-1, 1])], axis=1)
